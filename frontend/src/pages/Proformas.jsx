...
import { 
  Download, Share2, Plus, Search, FileCheck, Trash2,
...
const Proformas = ({ user }) => {
  ...
  const fetchProformas = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/proformas`);
  ...
  const handleDownload = async (id, number) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/proformas/${id}/pdf`, { responseType: 'blob' });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `PI_${number}.pdf`);
  ...
  const handleView = async (id) => {
    try {
      ...
      alert('Failed to preview proforma invoice.');
    }
  };

  const handleConvertToInvoice = async (id) => {
    if (!window.confirm("Are you sure you want to convert this Proforma to an Invoice?")) return;
    setConverting(true);
    try {
      const res = await axios.post(`${API_BASE_URL}/proformas/${id}/convert`, {});
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this proforma? This action cannot be undone.")) return;
    try {
      await axios.delete(`${API_BASE_URL}/proformas/${id}`);
      fetchProformas();
    } catch (err) {
  ...
  const filteredProformas = proformas.filter(p => {
    ...
    const pNum = p.proforma_number.toLowerCase();
    const matchesSearch = cName.includes(term) || pNum.includes(term);
  ...
  const totalVolume = proformas.reduce((sum, p) => sum + (p.total_amount || 0), 0);
  const convertedVolume = proformas.filter(p => p.status === 'CONVERTED').reduce((sum, p) => sum + (p.total_amount || 0), 0);
      ...
      <div style={{ backgroundColor: '#ffffff', border: '1px solid #eaedf3', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.01)', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid #eaedf3', padding: '0 32px', backgroundColor: '#fafafa' }}>
          {['ALL', 'DRAFT', 'CONVERTED'].map((tab) => (
            ...
            >
              {tab === 'ALL' ? 'All Proformas' : tab === 'DRAFT' ? 'Drafts' : 'Converted'}
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ backgroundColor: '#f8fafc', borderBottom: '1px solid #eaedf3' }}>
                <th style={{ padding: '16px 32px', textAlign: 'left', fontSize: '12px', fontWeight: '700', color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Proforma</th>
            ...
            <tbody>
              {filteredProformas.length === 0 ? (
                <tr>
                  <td colSpan="6" style={{ padding: '60px 32px', textAlign: 'center' }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                      ...
                      <div>
                        <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#111827', margin: '0 0 4px 0' }}>No proformas found</h3>
                        <p style={{ fontSize: '14px', color: '#6b7280', margin: 0 }}>Create a new proforma invoice to get started.</p>
                      </div>
                    </div>
                  </td>
                </tr>
                ...
                filteredProformas.map((p) => (
                  <tr key={p.id} style={{ borderBottom: '1px solid #eaedf3', transition: 'background-color 0.15s ease' }} onMouseEnter={e => e.currentTarget.style.backgroundColor = '#f8fafc'} onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}>
                    <td style={{ padding: '16px 32px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '14px', fontWeight: '700', color: '#111827' }}>{p.proforma_number}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        ...
                        <span style={{ fontSize: '14px', fontWeight: '600', color: '#374151' }}>{p.company_name}</span>
                      </div>
                    </td>
                    <td style={{ padding: '16px' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: '#4b5563', fontWeight: '500' }}>
                          <Calendar size={13} style={{ color: '#9ca3af' }} /> {new Date(p.date).toLocaleDateString('en-GB')}
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '16px', textAlign: 'center' }}>
                      <span style={{
                        ...
                        backgroundColor: p.status === 'CONVERTED' ? '#ecfdf5' : '#f3f4f6',
                        color: p.status === 'CONVERTED' ? '#10b981' : '#4b5563'
                      }}>
                        {p.status === 'CONVERTED' && <CheckCircle size={14} />}
                        {p.status}
                      </span>
                    </td>
                    <td style={{ padding: '16px 32px', textAlign: 'right' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '8px' }}>
                        <button onClick={() => handleView(p.id)} style={{ padding: '6px 10px', backgroundColor: '#f1f5f9', border: '1px solid #e2e8f0', borderRadius: '6px', color: '#475569', fontSize: '13px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }} onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#e2e8f0'; e.currentTarget.style.color = '#0f172a'; }} onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#f1f5f9'; e.currentTarget.style.color = '#475569'; }}>
                          <Eye size={14} /> View
                        </button>
          ...
          <div style={{ width: '100%', maxWidth: '1000px', height: '100%', maxHeight: '90vh', backgroundColor: '#ffffff', borderRadius: '16px', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' }}>
            <div style={{ padding: '16px 24px', borderBottom: '1px solid #eaedf3', display: 'flex', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#fafafa' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                ...
                <div>
                  <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#111827', margin: 0 }}>Proforma: {selectedProf.proforma_number}</h3>
                  <p style={{ fontSize: '12px', color: '#6b7280', margin: 0 }}>Preview Document</p>
                </div>
              </div>
              <div style={{ display: 'flex', gap: '12px' }}>
                <button onClick={() => handleDownload(selectedProf.id, selectedProf.proforma_number)} style={{ padding: '8px 16px', backgroundColor: 'var(--primary-color)', color: '#ffffff', border: 'none', borderRadius: '6px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ flex: 1, backgroundColor: '#f1f5f9', padding: '24px' }}>
              <iframe src={previewUrl} style={{ width: '100%', height: '100%', border: 'none', borderRadius: '8px', backgroundColor: '#ffffff', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }} title="Proforma PDF Preview" />
            </div>
          </div>
        </div>
      )}
